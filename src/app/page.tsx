...
export default function Dashboard() {
  ...
  const totalHours = logs.reduce((acc, log) => acc + (log.hours || 8), 0);
  const totalDays = (totalHours / 8).toFixed(1);
  const totalMonths = (parseFloat(totalDays) / 22).toFixed(1); // Approx 22 working days/month
  ...
  const handleUpdateLog = async (id: string, newDate: string, newDescription: string, newHours: number) => {
    try {
      ...
      setEditingLog(null);
      // If we were viewing this log, update the view as well (optional, but good UX)
      if (viewingLog && viewingLog.id === id) {
        setViewingLog({ ...viewingLog, date: newDate, description: newDescription, hours: newHours });
      }
    } catch (error) {
          ...
          <div className="lg:col-span-2">
            ...
            <div className="glass-panel p-4 rounded-2xl mb-6 flex items-center justify-between gap-4 border border-white/10 bg-gradient-to-r from-blue-500/5 to-purple-500/5">
              ...
              <div className="flex flex-col items-center flex-1">
                ...
                <span className="text-xl md:text-2xl font-bold text-blue-400">{totalDays}</span>
              </div>
              <div className="w-px h-8 bg-white/10"></div>
              <div className="flex flex-col items-center flex-1">
                <span className="text-xs text-slate-400 uppercase tracking-wider mb-1">{t('months')}</span>
                <span className="text-xl md:text-2xl font-bold text-purple-400">{totalMonths}</span>
              </div>
            </div>
